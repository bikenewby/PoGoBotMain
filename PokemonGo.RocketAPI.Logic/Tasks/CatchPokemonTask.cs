        ...
        public static async Task Execute(dynamic encounter, MapPokemon pokemon, FortData currentFortData = null, ulong encounterId = 0)
        {
            ...
            do
            {
                ...
                if (encounter?.CaptureProbability?.CaptureProbability_ != null)
                {
                    ...
                    Logger.Write($"({catchStatus} / {catchType}) | ({Logic._client.CurrentLatitude},{Logic._client.CurrentLongitude}) {Id} - Lvl {Level} [CP {Cp}/{MaxCp} | IV: {Iv.ToString("0.00")}% perfect] | Chance: {Probability} | {distance:0.##}m dist | with a {returnRealBallName(pokeball)}Ball [Remaining: {BallAmount}] {receivedXp}", LogLevel.Pokemon);
                }
